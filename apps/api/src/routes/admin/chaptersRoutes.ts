...
export function registerChapterRoutes(admin: Hono<{ Bindings: Env; Variables: Variables }>) {
  ...
  admin.post('/chapters', async (c) => {
    ...
    const row = await c.env.DB.prepare(
      `INSERT INTO chapters (title_mn, subtitle_mn, color, hsk_level, order_num, is_published, flashcard_delay_days)
       VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id`
    )
      .bind(
        title,
        body.subtitle_mn ?? '',
        body.color ?? '#58CC02',
        Math.min(6, Math.max(1, Number(body.hsk_level ?? 1))),
  ...
  admin.patch('/chapters/:id', async (c) => {
    ...
    await c.env.DB.prepare(
      ...
      .bind(
        ...
        body.color ?? null,
        body.hsk_level !== undefined ? Math.min(6, Math.max(1, Number(body.hsk_level))) : null,
